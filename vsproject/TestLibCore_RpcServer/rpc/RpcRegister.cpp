...
namespace Server
{
	...
	void RpcInstance::OnRegisterRpcs( void )
	{
		...
		//5 TestObject generate default deliver and return check param here
		{
			...
			m_pRpcServerManager->RegisterFunc<Msg::GlobalRpc >(Msg::g_szTestObject_RpcServer , &Msg::GlobalRpc::TestObject_RpcServer); 
		}
		...
		//5 TestObject1 generate default deliver and return check param here
		{
			...
			objDeliverParams.Clear();
			objReturnParams.Clear();
			m_pRpcServerManager->RegisterFunc<Msg::GlobalRpc >(Msg::g_szTestObject1_RpcServer , &Msg::GlobalRpc::TestObject1_RpcServer); 
		}
