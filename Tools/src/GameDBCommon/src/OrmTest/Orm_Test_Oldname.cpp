...
namespace Orm
{
	...
	void Test_Oldname::ToCompress(std::string & __buf)
	{
		mongo::BSONObj  __obj;
		ToBson(__obj);
	...
	void Test_Oldname::ToBson(std::string & __buf)
	{
		mongo::BSONObj  __obj;
		ToBson(__obj);
	...
	void Test_Oldname::ToBson(mongo::BSONObj  & __obj)
	{
	...
	void Test_Oldname::FromCompress(const std::string& __inbuf)
	{
		std::string tmpbuf;
	...
	void Test_Oldname::FromCompress(const char* __data,INT32 __size)
	{
		std::string tmpbuf;
	...
	void Test_Oldname::FromBson(const char* __data,INT32 __size)
	{
	...
	void Test_Oldname::FromBson(const mongo::BSONObj  & __obj)
	{
	...
	int Test_Oldname::Getid() const
	{
		return id;
	}

	void Test_Oldname::Setid(int & value)
	{
		id = value;
	}
