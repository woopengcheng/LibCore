...
namespace Server
{
	...
	{
		GEN_RPC_CALL_1((&(Server::DBMaster::GetInstance())) , pSessionName , Msg::g_szMasterStartSync_RpcCall , value,  vecTargets , objSrc , usPriority , Server::DBMaster::GetInstance().GetServerName() , objSyncType);
	}
	...
	{
		GEN_RPC_CALL_0((&(Server::DBSlave::GetInstance())) , pSessionName , Msg::g_szSlaveRequestSync_RpcCall ,  vecTargets , objSrc , usPriority , Server::DBSlave::GetInstance().GetServerName() , objSyncType);
	}
