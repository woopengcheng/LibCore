...
namespace Server
{
	...
	void DBSlave::OnRegisterRpcs( void )
	{
		...
		{
			Msg::GenMsgHelper::GenMsgParams(objDeliverParams  , g_rpcDefaultParam_std_string , g_rpcDefaultParam_INT64 , g_rpcDefaultParam_std_string);
	...
	void DBMaster::OnRegisterRpcs( void )
	{
		...
		//5 SlaveStartAuth generate default deliver and return check param here
		{
			...
			objDeliverParams.Clear();
			objReturnParams.Clear();
			m_pRpcServerManager->RegisterFunc<MasterHandler >(Msg::g_szSlaveStartAuth_RpcServer , &MasterHandler::SlaveStartAuth_RpcServer); 
		}
